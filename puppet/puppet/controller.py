...
class MotorControllerROS2(Node):
    ...
    def send_motor_rpm(self, m0, m1, m2, m3):
        ...
        try:
            ...
            # Pack data (signed 16-bit big-endian)
            data = bytearray(8)
            for i in range(4):
                rpm = motors[i]
                if rpm < 0:
                    rpm = 65536 + rpm  # Two's complement
                data[i*2] = (rpm >> 8) & 0xFF
                data[i*2 + 1] = rpm & 0xFF
            
            # Send message
            message = can.Message(arbitration_id=self.can_tx_id, data=data)
        ...
        try:
            ...
            if message and message.arbitration_id == self.can_rx_id:
                if len(message.data) == 8:
                    for i in range(4):
                        raw_rpm = (message.data[i*2] << 8) | message.data[i*2 + 1]
                        if raw_rpm > 32767:
                            raw_rpm -= 65536
